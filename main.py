...
class Orchestrator:
    ...
    def run(self, company: str = "Stripe", industry: str = "fintech", api_key: str = None) -> dict:
        """
        ...
        The returned state dict contains:
          ...
          - status: "complete"
        """
        explicit_api_key = api_key.strip() if api_key else self.api_key
        return run_analysis(company=company, industry=industry, api_key=explicit_api_key)
