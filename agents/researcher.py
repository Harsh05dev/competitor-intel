    ...
    def research(self, state: AgentState) -> Dict[str, Any]:
        ...
        if iteration > 0 and state.get("research_results"):
            ...
            for result in state["research_results"]:
                name = (result.get("company_name") or "").strip().lower()
                if not name:
            ...
            for new_comp in data:
                ...
                new_snippets = self._as_list(new_comp.get("raw_snippets"))
                new_sources = self._as_list(new_comp.get("sources"))
                if key in existing:
                    # Append new snippets to existing ones, deduplicate
                    combined = existing[key]["raw_snippets"] + new_snippets
                    existing[key]["raw_snippets"] = list(dict.fromkeys(combined))
                    existing[key]["sources"] = list(dict.fromkeys(existing[key]["sources"] + new_sources))
                elif key:
                    existing[key] = {
                        **new_comp,
