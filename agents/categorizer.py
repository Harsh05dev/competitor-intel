...
class CategorizerAgent:
    ...
    def _merge(self, existing: List[dict], new_data: List[dict]) -> List[dict]:
        ...
        """
        new_map = {self._company_key(c): c for c in new_data if self._company_key(c)}
        merged  = []
        ...
        existing_keys = {self._company_key(c) for c in existing if self._company_key(c)}
    ...
    @staticmethod
    def _company_key(comp: dict) -> str:
        name = comp.get("company_name") if isinstance(comp, dict) else None
