...
# Each analysis run owns fresh agents, which prevents user-supplied Streamlit
# keys from leaking across sessions through shared module-level clients.

def _create_node_functions(api_key: Optional[str] = None):
...
def build_graph(api_key: Optional[str] = None):
    ...
    """
    researcher_node, categorizer_node, analyst_node, evaluator_node = _create_node_functions(api_key=api_key)
