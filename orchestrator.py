...
def _new_agents(api_key: str | None = None):
    return (
...
def _get_agents():
    ...
    if _researcher is None:
        _researcher, _categorizer, _analyst, _evaluator = _new_agents()
...
def researcher_node(state: AgentState) -> dict:
    ...
    return _researcher_node(state, researcher)
...
def categorizer_node(state: AgentState) -> dict:
    _, categorizer, _, _ = _get_agents()
    return _categorizer_node(state, categorizer)
...
def analyst_node(state: AgentState) -> dict:
    _, _, analyst, _ = _get_agents()
    return _analyst_node(state, analyst)
...
def evaluator_node(state: AgentState) -> dict:
    _, _, _, evaluator = _get_agents()
    return _evaluator_node(state, evaluator)
...
def build_graph(api_key: str | None = None):
    ...
    """
    if api_key:
        researcher, categorizer, analyst, evaluator = _new_agents(api_key=api_key)
    else:
