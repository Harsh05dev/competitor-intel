...
def researcher_node(state: AgentState, researcher: Optional[ResearcherAgent] = None) -> dict:
    ...
    result = (researcher or _researcher).research(state)
...
def categorizer_node(state: AgentState, categorizer: Optional[CategorizerAgent] = None) -> dict:
    ...
    result = (categorizer or _categorizer).categorize(state)
...
def analyst_node(state: AgentState, analyst: Optional[AnalystAgent] = None) -> dict:
    ...
    result = (analyst or _analyst).analyze(state)
...
def evaluator_node(state: AgentState, evaluator: Optional[EvaluatorAgent] = None) -> dict:
    ...
    result = (evaluator or _evaluator).evaluate(state)
...
def build_graph(api_key: Optional[str] = None):
    ...
    # ── Register nodes ─────────────────────────────────────────────────────────
    graph.add_node("researcher",   partial(researcher_node, researcher=researcher))
    graph.add_node("categorizer",  partial(categorizer_node, categorizer=categorizer))
    graph.add_node("analyst",      partial(analyst_node, analyst=analyst))
    graph.add_node("evaluator",    partial(evaluator_node, evaluator=evaluator))
