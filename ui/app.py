...
from app_utils import escape_html as h, report_to_pdf_bytes
...
def render_competitors(competitors):
    ...
    for c in competitors:
        sh = "".join(f'<div class="snippet">· {h(s)}</div>' for s in c.get("raw_snippets", []))
        sr = "".join(f'<div class="src">↗ {h(s)}</div>' for s in c.get("sources", [])[:2])
        st.markdown(f'<div class="comp-card"><div class="comp-name"><span class="c-dot"></span>{h(c.get("company_name","?"))}</div>{sh}{sr}</div>', unsafe_allow_html=True)
...
def render_swot(analysis):
    ...
    for i, (label, key, color) in enumerate([("Strengths 💪","strengths",ACCENT),("Weaknesses ⚠️","weaknesses",RED),("Opportunities 🚀","opportunities",BLUE),("Threats 🔴","threats",AMBER)]):
        rows = "".join(f'<div style="font-size:0.8rem;color:{TEXT2};padding:0.25rem 0 0.25rem 0.7rem;border-left:2px solid {color}40;margin-bottom:0.25rem;line-height:1.45;">· {h(item)}</div>' for item in swot.get(key, []))
...
def render_comparison(analysis):
    ...
    for row in matrix:
        t = str(row.get("threat_level") or "Low")
        tc = RED if t == "High" else AMBER if t == "Medium" else TEXT3
        st.markdown(f'<div class="comp-card"><div class="comp-name"><span class="c-dot"></span>{h(row.get("company_name","?"))} <span style="font-family:Space Mono,monospace;font-size:0.57rem;color:{tc};margin-left:auto;">▲ {h(t.upper())} THREAT</span></div><div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:0.5rem;font-size:0.8rem;color:{TEXT2};"><div><span style="color:{TEXT3};font-size:0.58rem;font-family:Space Mono,monospace;display:block;margin-bottom:0.12rem;">PRICING</span>{h(row.get("pricing_tier","?"))}</div><div><span style="color:{TEXT3};font-size:0.58rem;font-family:Space Mono,monospace;display:block;margin-bottom:0.12rem;">STRENGTH</span>{h(row.get("primary_strength","?"))}</div><div><span style="color:{TEXT3};font-size:0.58rem;font-family:Space Mono,monospace;display:block;margin-bottom:0.12rem;">MARKET</span>{h(row.get("target_market","?"))}</div></div></div>', unsafe_allow_html=True)

def render_gaps(gaps, queries):
    gc, qc = st.columns(2)
    with gc:
        st.markdown('<div class="sec-head">Data Gaps</div>', unsafe_allow_html=True)
        for g in (gaps or []): st.markdown(f'<div class="gap-row">⚠ {h(g)}</div>', unsafe_allow_html=True)
    ...
    with qc:
        ...
        for q in (queries or []): st.markdown(f'<div class="q-row">→ {h(q)}</div>', unsafe_allow_html=True)
...
def run_demo_mode(company, industry):
    ...
    for msg, pct, delay in [
        ...
        slot.markdown(f'<div class="prog-row"><div class="prog-dot"></div>{h(msg)}</div>', unsafe_allow_html=True)
...
def render_result(result, company_for_filename):
    ev = result.get("evaluation", {})
    ...
    with t4:
        ...
        for log in result.get("logs", []):
            ...
            st.markdown(f'<div style="font-family:Space Mono,monospace;font-size:0.68rem;color:{color};padding:0.17rem 0;border-bottom:1px solid {BORDER}33;">{h(log)}</div>', unsafe_allow_html=True)
    ...
    try:
        ...
        if report_md:
            st.download_button(
                "⬇ Download PDF Report",
                data=report_to_pdf_bytes(report_md),
                file_name=f"{company_for_filename or 'company'}_competitor_report.pdf",
...
if run_btn:
    ...
    st.session_state.last_run_company = company_final
    st.session_state.last_run_industry = industry_final

if st.session_state.last_result:
    render_result(st.session_state.last_result, st.session_state.last_run_company)
else:
