...
# ── Session defaults ───────────────────────────────────────────────────────────
for k, v in [("theme", "dark"), ("mode", "demo"), ("gemini_api_key", "")]:
...
if run_btn:
    ...
    if DEMO:
        ...
        api_key_for_run = st.session_state.get("gemini_api_key", "").strip()
        if not api_key_for_run:
            st.error("Paste your Gemini API key in the sidebar first."); st.stop()
        try:
            result = run_live_mode(company_final, industry_final, api_key_for_run)
