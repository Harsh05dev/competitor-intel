...
if run_btn:
    ...
    if DEMO:
        ...
        live_api_key = api_key.strip() if api_key else None
        if not live_api_key and not os.environ.get("GEMINI_API_KEY", ""):
            st.error("Paste your Gemini API key in the sidebar first."); st.stop()
        try:
            result = run_live_mode(company_final, industry_final, api_key=live_api_key)
